                ...
                (!["table", 'none'].includes(display) || (display === 'none' && superS)) && <div className={!(hasSubs || noGrid) ? 'grid gap-x-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4': undefined }>
                    {
                        content?.filter(e => !e?.noDesc).map((e, i) => (
                            typeof e === 'string' ? <p key={i} className="col-span-1 md:col-span-2 lg:col-span-3 xl:col-span-4">{<Parser text={e} passedSection={thisName}/>}</p> : hasSubs ? <Section key={i} data={e} superS={thisName}/> : <Item key={i} data={e} section={thisName}/>
                        ))
                    }
                </div>
            }
